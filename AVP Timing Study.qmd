...
Multicollinearity of included confounders was assessed with pairwise Pearson’s correlation coefficients and variance inflation factors and no multicollinearity was detected
...
ggplot(pred.neq) +
  ...
  ylab('Predicted rate of in-hospital mortality, %')+
...
##### Norepinephrine-Equivalent Dose at Vasopressin Initiation
*Adjusting the units of the variable from 1 to 10 mcg/min*\
Slope 1 is the OR with 95% CI up until the change point of 39.5 mcg/min. Slope 2 is the OR with 95% CI for all doses after the change point of 39.5 mcg/min. The OR reference unit is for every 10 mcg/min increase in the NEQ dose at the time of AVP initiation.  
```{r}
...
##### Lactate Concentration at Vasopressin Initiation
*The units of lactate are 1 mmol/L*\
The first line of code is the odds ratio for the lactate concentration at AVP initiation with the second and third line being the lower and upper bound of the 95% confidence interval. 
```{r}
#| warning: false
#| message: false
model <- summary.segmented(segavptime)
exp(model$coefficients[3,1])
exp(model$coefficients[3,1] - 1.96*model$coefficients[3,2]) # lower bound of 95% CI
exp(model$coefficients[3,1] + 1.96*model$coefficients[3,2]) # upper bound of 95% CI
```
...
ggplot(pred.neq) +
  ...
  geom_ribbon(aes(x=neqdoseatavp, ymin=prob.lower, ymax=prob.upper), alpha=0.3) + 
  xlab('Norepinephrine-equivalent dose at vasopressin initiation, mcg/min') + 
  ylab('Predicted probability of in-hospital mortality, %')+
  scale_x_continuous(breaks=seq(0,160,by=20), limits=c(0,160)) +
  scale_y_continuous(breaks=seq(.2,1,by=.1), limits=c(.2,1), labels = scales::percent_format(accuracy = 1)) + 
  theme_light()
```
...
### Segmented Regression
Similar to methods previously preformed in our initial study, piecewise segmented regression was utilized to determine the breakpoint at which the relationship between norepinephrine-equivalent dose at vasopressin initiation and in-hospital mortality changed.  This method estimates the change point of slope in NEQ-dose at AVP initiation, or the point at which the odds ratio of the association between NEQ-dose at AVP initiation and in-hospital mortality changes. The change point was detected at a NEQ dose of 37.9 mcg/min. This method utilized the segmented package and the [segmented function in R](https://cran.r-project.org/web/packages/segmented/segmented.pdf)
```{r}
segavptime2 <- segmented(avptime2, ~neqdoseatavp, control=seg.control(size.boot = 8000, display=TRUE, seed=1234))
...
##### Norepinephrine-Equivalent Dose at Vasopressin Initiation
*Adjusting the units of the variable from 1 to 10 mcg/min*\
Slope 1 is the OR with 95% CI up until the change point of 37.2 mcg/min. Slope 2 is the OR with 95% CI for all doses after the change point of 39 mcg/min. The OR reference unit is for every 10 mcg/min increase in the NEQ dose at the time of AVP initiation.  
```{r}
...
exp(model2$coefficients[3,1])
exp(model2$coefficients[3,1] - 1.96*model2$coefficients[3,2]) # lower bound of 95% CI
exp(model2$coefficients[3,1] + 1.96*model2$coefficients[3,2]) # upper bound of 95% CI
```
...
cubavptime3 <- lrm(inhospital_mortality~ rcs(neqdoseatavp,3) + lactateatavp + timefromshockonset_hr + age + weight + mv_baseline + akimva + sofa_baseline + totalfluidbolus + fluidbalance + gender + race + immune_suppression + database + iculocation + hydrocortuse, data = fulldfsscoding, x=TRUE, y=TRUE)
summary(cubavptime3)
```

##### Values of where splines placed knots
We can see the splines placed their knots at NEQ dose at AVP initiation of 9.1, 27.2, and 70.3 mcg/min
```{r}
knots <- attributes(rcs(fulldfsscoding$neqdoseatavp,3))
knots$parms
```

#### Estimated Odds Ratios
The reported estimated odds ratios for the final regression model utilizing cubic splines with three knots for NEQ dose at AVP initiation are reported below. Rather than using the clinically relevant breakpoints that were utilized in the original vasopressin timing evaluation (10, 25, and 60 mcg/min), will utilize the breakpoints detected by the knots in the cubic splines regression (9, 27, and 70 mcg/min)
...
```{r}
summary(cubavptime3, neqdoseatavp)[c(40*3+2,40*5+2,40*6+2)]
```

Odds ratio comparing 27 mcg/min to 9 mcg/min. 
```{r}
summary(cubavptime3, neqdoseatavp=c(9,27))[c(40*3+2,40*5+2,40*6+2)]
```

Odds ratio comparing 70 mcg/min 9 mcg/min.
```{r}
summary(cubavptime3, neqdoseatavp=c(9,70))[c(40*3+2,40*5+2,40*6+2)]
```

Odds ratio comparing 70 mcg/min to 27 mcg/min
```{r}
summary(cubavptime3, neqdoseatavp=c(27,70))[c(40*3+2,40*5+2,40*6+2)]
```

Odds ratio for lactate concentration
```{r}
summary(cubavptime3, lactateatavp=c(1,2))[c(40*3+4,40*5+4,40*6+4)]
```

Odds ratio for time from shock onset
```{r}
summary(cubavptime3, timefromshockonset_hr=c(1,2))[c(40*3+6,40*5+6,40*6+6)]
```
...
The predicted probability of in-hospital mortality when vasopressin is initiated at a dose of 10 mcg/min of norepinephrine is 48.1% (95% CI 37.0-59.4%).

```{r}
probneq10 <- data.frame(Predict(cubavptime3, type = "predictions", neqdoseatavp = 10, lactateatavp = 3.5, timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
The predicted probability of in-hospital mortality when vasopressin is initiated at a dose of 25 mcg/min of norepinephrine is 55.7% (95% CI 45.4-65.5%).
```{r}
probneq25 <- data.frame(Predict(cubavptime3, type = "predictions", neqdoseatavp = 25, lactateatavp = 3.5, timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
The predicted probability of in-hospital mortality when vasopressin is initiated at a dose of 60 mcg/min of norepinephrine is 67.6% (95% CI 57.6-76.1%).
```{r}
probneq60 <- data.frame(Predict(cubavptime3, type = "predictions", neqdoseatavp = 60, lactateatavp = 3.5, timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
#| warning: false
#| message: false
pred.neq <- as.data.frame(Predict(cubavptime3, type = "predictions", neqdoseatavp, lactateatavp = 3.5, timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
#| warning: false
#| message: false
pred.lac <- as.data.frame(Predict(cubavptime3, type = "predictions", lactateatavp, neqdoseatavp = 27.2, timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
#| warning: false
#| message: false
pred.time <- as.data.frame(Predict(cubavptime3, type = "predictions", timefromshockonset_hr, lactateatavp=3.5, neqdoseatavp = 27.2, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV", sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male"))
...
### Segmented Regression
Similar to methods previously preformed in our initial study, piecewise segmented regression was utilized to determine the breakpoint at which the relationship between norepinephrine-equivalent dose at vasopressin initiation and in-hospital mortality changed.  This method estimates the change point of slope in NEQ-dose at AVP initiation, or the point at which the odds ratio of the association between NEQ-dose at AVP initiation and in-hospital mortality changes. The change point was detected at a NEQ dose of 42.281 mcg/min. This method utilized the segmented package and the [segmented function in R](https://cran.r-project.org/web/packages/segmented/segmented.pdf)

However, you can see that the 95% CI for the change point crosses zero and thus does not make clinical sense. Because of this, would not pursue segmented regression for the population based on septic shock diagnostic coding.
```{r}
...
##### Norepinephrine-Equivalent Dose at Vasopressin Initiation
*Adjusting the units of the variable from 1 to 10 mcg/min*\
Slope 1 is the OR with 95% CI up until the change point of 42.28 mcg/min. Slope 2 is the OR with 95% CI for all doses after the change point of 42.28 mcg/min. The OR reference unit is for every 10 mcg/min increase in the NEQ dose at the time of AVP initiation.  
```{r}
segoutput3 <- as.data.frame(slope(segavptime3)) %>%
  mutate(OR = exp(neqdoseatavp.Est.)^10,
         LB95CI = exp(neqdoseatavp.CI.95...l)^10,
         UB95CI = exp(neqdoseatavp.CI.95...u)^10) %>%
  select(OR, LB95CI, UB95CI)
...
The first line of code is the odds ratio for the lactate concentration at AVP initiation with the second and third line being the lower and upper bound of the 95% confidence interval.
```{r}
model3 <- summary.segmented(segavptime3)
exp(model3$coefficients[3,1])
exp(model3$coefficients[3,1] - 1.96*model3$coefficients[3,2]) # lower bound of 95% CI
exp(model3$coefficients[3,1] + 1.96*model3$coefficients[3,2]) # upper bound of 95% CI
```
...
The first line of code is the odds ratio for the time between shock onset and AVP initiation with the second and third line being the lower and upper bound of the 95% confidence interval.
```{r}
exp(model3$coefficients[4,1])
exp(model3$coefficients[4,1] - 1.96*model3$coefficients[4,2]) # lower bound of 95% CI
exp(model3$coefficients[4,1] + 1.96*model3$coefficients[4,2]) # upper bound of 95% CI
```
...
```{r}
pred.neq <- data.frame(neqdoseatavp = seq(0,200,by = 0.01))
pred.neqdf <- data.frame(neqdoseatavp = seq(0,200,by = 0.01), lactateatavp = 3.5,
                         timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV",
                         sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", 
                         iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male")

pred.neq.1 <- predict(segavptime3, newdata =pred.neqdf, se.fit=T)
pred.neq$prob <- logistic(pred.neq.1$fit)
pred.neq$prob.lower <- logistic(pred.neq.1$fit-1.96*pred.neq.1$se.fit)
pred.neq$prob.upper <- logistic(pred.neq.1$fit+1.96*pred.neq.1$se.fit)
...
# setting values as median unless otherwise specified
pred.lacdf <- data.frame(lactateatavp = seq(0,30,by = 0.01), neqdoseatavp = 27.2,
                         timefromshockonset_hr = 5.8, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV",
                         sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", 
                         iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male")

pred.lac.1 <- predict(segavptime3, newdata = pred.lacdf, se.fit=TRUE)
logistic <- plogis
pred.lac$prob <- logistic(pred.lac.1$fit)
pred.lac$prob.lower <- logistic(pred.lac.1$fit-1.96*pred.lac.1$se.fit)
pred.lac$prob.upper <- logistic(pred.lac.1$fit+1.96*pred.lac.1$se.fit)
...
# setting values as median unless otherwise specified
pred.timedf <- data.frame(timefromshockonset_hr = seq(0,48,by = 0.01), neqdoseatavp = 27.2,
                          lactateatavp = 3.5, age=65, weight=80, akimva = "No AKI", database = "MIMIC-IV",
                          sofa_baseline=9, totalfluidbolus = 1000, fluidbalance=9700, immune_suppression="No", 
                          iculocation = "MICU", hydrocortuse = "Yes", mv_baseline = "Yes", race = "White", gender = "Male")

pred.time.1 <- predict(segavptime3, newdata = pred.timedf, se.fit=TRUE)
logistic <- plogis
pred.time$prob <- logistic(pred.time.1$fit)
pred.time$prob.lower <- logistic(pred.time.1$fit-1.96*pred.time.1$se.fit)
pred.time$prob.upper <- logistic(pred.time.1$fit+1.96*pred.time.1$se.fit)
...
```{r}
segavptime4 <- segmented(avptime4, ~neqdoseatavp, control=seg.control(size.boot = 8000, display=TRUE, seed=1234))
...
Moving forward, for the Angus criteria, will not report out results of the segmented regression. 
